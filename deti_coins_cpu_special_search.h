...
static void deti_coins_cpu_special_search(u32_t n_random_words)
{
    ...
    #pragma omp parallel reduction(+:n_coins, n_attempts)
    {
        ...
        for (int lane = 0; lane < 8; lane++) {
            ...
            bytes[lane][8] = 'n'; bytes[lane][9] = ' '; bytes[lane][10] = ' '; bytes[lane][11] = 'M';
            bytes[lane][12] = 'a'; bytes[lane][13] = 'r'; bytes[lane][14] = 'c'; bytes[lane][15] = 'o';


            for (int word_idx = 0; word_idx < 7; word_idx++) {
                int idx = 16 + word_idx * 4; // Calcula o índice inicial do byte correspondente
            ...
            bytes[lane][44] = 'G'; bytes[lane][45] = 'a'; bytes[lane][46] = 'b'; bytes[lane][47] = 'r';
            bytes[lane][48] = 'i'; bytes[lane][49] = 'e'; bytes[lane][50] = 'l'; bytes[lane][51] = '\n';
        }
        ...
        while (stop_request == 0) {
            ...
            for (int lane = 0; lane < 8; lane++) {
                ...
                for (idx = 16u; idx < 51u && bytes[lane][idx] == (u08_t)126; idx++)
                    bytes[lane][idx] = ' ';
                if (idx < 52u - 1u)
                    bytes[lane][idx]++;
            }
